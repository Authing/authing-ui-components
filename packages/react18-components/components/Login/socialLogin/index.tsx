...
export const SocialLogin: React.FC<SocialLoginProps> = ({
  ...
  const socialLoginButtons = socialConnectionObjs.map((item: any) => {
    ...
    const onLogin = () => {
      const loginErrorMsg = t('login.socialConnectionMessage', {
        provider:
          item.displayName ??
  ...
  const idpMsg = i18n.t('login.otherLoginWay')

  const idp = enterpriseConnectionObjs.length ? (
    <>
      {!noLoginMethods && <div className="g2-social-login-title">{idpMsg}</div>}
      <div className="g2-social-login-list">{idpButtons}</div>
    </>
  ) : null

  const thridAccLoginMsg = t('login.thridAccLogin')
  ...
  const socialLogin =
    socialLoginButtons.length > 0 && noLoginMethods ? (
      ...
      socialLoginButtons.length > 0 && (
        <>
          <div className="g2-social-login-title">{thridAccLoginMsg}</div>
